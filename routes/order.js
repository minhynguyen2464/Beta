        ...
        if (rspCode == "00") { //Thêm vào
            ...
            Booking.findOneAndUpdate(query, update, options)
                .then(() => {
                    ...
                    Booking.findById({ _id: orderId }).exec() //Query để tìm movie id
                        .then((data) => {
                            Movie.findById(data.movie) //Query để tìm showtime
                                .then((movie) => {
                                    const showtime = movie.showtimes.find(showtime => showtime._id.equals(data.showtimes)); //3 Query
